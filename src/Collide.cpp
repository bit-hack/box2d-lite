  ...
  const Vec2& normal, float offset, char clipEdge)
{
  ...
  const float distance0 = Dot(normal, vIn[0].v) - offset;
  const float distance1 = Dot(normal, vIn[1].v) - offset;
  ...
  const Mat22& Rot, const Vec2& normal)
{
  ...
  const Mat22 RotT = Rot.Transpose();
  const Vec2 n = -(RotT * normal);
  const Vec2 nAbs = Abs(n);
...
{
  // Setup
  const Vec2 hA = 0.5f * bodyA->width;
  const Vec2 hB = 0.5f * bodyB->width;
  ...
  const Vec2 dp = posB - posA;
  const Vec2 dA = RotAT * dp;
  const Vec2 dB = RotBT * dp;

  const Mat22 C = RotAT * RotB;
  const Mat22 absC = Abs(C);
  const Mat22 absCT = absC.Transpose();
  ...
  case FACE_A_X:
  {
    ...
    const float side = Dot(posA, sideNormal);
    negSide = -side + hA.y;
    posSide = side + hA.y;
    negEdge = EDGE3;
    posEdge = EDGE1;
  ...
  case FACE_B_X:
  {
    ...
    const float side = Dot(posB, sideNormal);
    negSide = -side + hB.y;
    posSide = side + hB.y;
    negEdge = EDGE3;
    posEdge = EDGE1;
  ...
  std::array<ClipVertex, 2> clipPoints1;
  std::array<ClipVertex, 2> clipPoints2;

  // Clip to box side 1
  const int np1 = ClipSegmentToLine(clipPoints1.data(), incidentEdge, -sideNormal, negSide, negEdge);
  if (np1 < 2)
    return 0;

  // Clip to negative box side 1
  const int np2 = ClipSegmentToLine(clipPoints2.data(), clipPoints1.data(), sideNormal, posSide, posEdge);
  if (np2 < 2)
    return 0;
