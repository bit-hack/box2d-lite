...
void Joint::Set(Body* b1, Body* b2, const Vec2& anchor)
{
  ...
  const Mat22 Rot1(body1->rotation);
  const Mat22 Rot2(body2->rotation);
  const Mat22 Rot1T = Rot1.Transpose();
  const Mat22 Rot2T = Rot2.Transpose();
...
void Joint::PreStep(float inv_dt)
{
  ...
  const Vec2 p1 = body1->position + r1;
  const Vec2 p2 = body2->position + r2;
  const Vec2 dp = p2 - p1;
...
{
  const Vec2 dv = body2->velocity + Cross(body2->angularVelocity, r2) - body1->velocity - Cross(body1->angularVelocity, r1);
  const Vec2 impulse = M * (bias - dv - softness * P);
