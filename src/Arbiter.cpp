...
void Arbiter::PreStep(float inv_dt)
{
  ...
  for (int i = 0; i < numContacts; ++i)
  {
    ...
    const Vec2 r1 = c.position - body1->position;
    const Vec2 r2 = c.position - body2->position;

    // Precompute normal mass, tangent mass, and bias.
    const float rn1 = Dot(r1, c.normal);
    const float rn2 = Dot(r2, c.normal);
    float kNormal = body1->invMass + body2->invMass;
    kNormal += body1->invI * (Dot(r1, r1) - rn1 * rn1) + body2->invI * (Dot(r2, r2) - rn2 * rn2);
    c.massNormal = 1.0f / kNormal;

    Vec2 tangent = Cross(c.normal, 1.0f);
    const float rt1 = Dot(r1, tangent);
    const float rt2 = Dot(r2, tangent);
...
void Arbiter::ApplyImpulse()
{
  ...
  for (int i = 0; i < numContacts; ++i)
  {
    ...
    const Vec2 tangent = Cross(c.normal, 1.0f);
    const float vt = Dot(dv, tangent);
    ...
    if (World::accumulateImpulses)
    {
      ...
      // Clamp friction
      const float oldTangentImpulse = c.Pt;
      c.Pt = Clamp(oldTangentImpulse + dPt, -maxPt, maxPt);
      dPt = c.Pt - oldTangentImpulse;
    }
    else
    {
      const float maxPt = friction * dPn;
      dPt = Clamp(dPt, -maxPt, maxPt);
    }

    // Apply contact impulse
    const Vec2 Pt = dPt * tangent;
